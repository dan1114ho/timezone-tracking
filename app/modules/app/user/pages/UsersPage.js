...
import { Table, Container, Dimmer, Loader, Button, Confirm } from 'semantic-ui-react';
  ...
  renderUsers = () => {
    ...
    return users.slice((page - 1) * pageSize, page * pageSize).map((user) => (
      <Table.Row key={user.get('_id')}>
        ...
        <Table.Cell>
          <Button color="teal" size="mini" as={Link} to={`/users/${user.get('_id')}`} content="Edit" icon="edit" labelPosition="left" />
          &nbsp;
          <Button color="orange" size="mini" content="Remove" icon="trash" labelPosition="left" onClick={this.onRemove(user.get('_id'))} />
        </Table.Cell>
      </Table.Row>
    ));
  }
    ...
    return (
      <Container>
        ...
        <div style={{ paddingTop: '20px', paddingBottom: '5px' }}>
          <span style={{ color: '#00b5ad', fontSize: '24px', fontWeight: 'bold' }}>Users</span>
          <Button color="teal" as={Link} to="/users/new" style={{ float: 'right' }}>Add New User</Button>
        </div>
        <Table celled>
          <Table.Header>
            <Table.Row>
